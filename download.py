...
#       <h1>Video Downloader</h1>
#       <div class="sub">
#         Paste a supported link, choose quality, then download to your PC or mobile browser.
#       </div>
...
#       <div class="note">
#         On mobile, the final button downloads the file through the browser. Most phones save it to Downloads, and many show it in Gallery automatically.
#       </div>
#     </div>
#   </div>
...
#           showStatus("Downloading...", data.message || "Downloading...", data.progress || 0);
#         } else if (data.status === "finished") {
#           showStatus("Download complete", data.message, 100);
#           if (data.file_url) {
#             finalDownloadBtn.href = data.file_url;
#             finalDownloadBtn.style.display = "inline-flex";
#             finalDownloadBtn.textContent = data.device_type === "mobile" ? "Save to Mobile" : "Save to PC";
...
#           submitBtn.disabled = false;
#         } else if (data.status === "error") {
#           showStatus("Download failed", data.message, 100);
#           finalDownloadBtn.style.display = "none";
#           clearInterval(poller);
#           submitBtn.disabled = false;
#         }
#       } catch (e) {
#         showStatus("Server error", "Could not read task status.", 100);
#         clearInterval(poller);
#         submitBtn.disabled = false;
#       }
#     }, 1200);
...
#       if (!res.ok) {
#         showStatus("Request failed", data.error || "Something went wrong.", 100);
#         submitBtn.disabled = false;
#         return;
#       }
...
#     } catch (err) {
#       showStatus("Server error", "Unable to connect to the server.", 100);
#       submitBtn.disabled = false;
#     }
#   });
# </script>
...
#         selector = format_selector(quality, ffmpeg_ready)
#         temp_id = str(uuid.uuid4())[:8]
#         temp_template = os.path.join(DOWNLOAD_DIR, f"{temp_id}_%(title).180s.%(ext)s")
...
#         if quality == "audio":
#             if not ffmpeg_ready:
#                 raise Exception("FFmpeg is required for audio conversion. Install FFmpeg on the server.")
#             ydl_opts["postprocessors"] = [{
#                 "key": "FFmpegExtractAudio",
#                 "preferredcodec": "mp3",
#                 "preferredquality": "192",
...
#             final_filename = unique_filename(title, ext)
#             final_path = os.path.join(DOWNLOAD_DIR, final_filename)
...
#             if not source_path.lower().endswith(f".{ext}"):
#                 renamed_source = source_path + f".{ext}"
#                 os.replace(source_path, renamed_source)
